...
if(isset($_POST['submit'])){
   ...
   if (empty($teacher) || empty($section) || empty($course) || empty($course_time)){
      ...
      $update_data = "UPDATE `section c` SET teacher='$teacher', section='$section', course='$course', course_time='$course_time', room='$room'  WHERE id = '$id'";
...
<div class="admin-product-form-container centered">
   ...
   <?php
      ...
      $select = mysqli_query($conn, "SELECT * FROM `section c` WHERE id = '$id'");
      while($row = mysqli_fetch_assoc($select)){

   ?>
   
   <form action="" method="post" enctype="multipart/form-data">
      <h3 class="title">update schedule section C</h3>
      ...
      <input type="text" class="box" name="room" value="<?php echo $row['room']; ?>" placeholder="enter the course_time">
